	...
	public void onClick(PlayerInteractEvent.LeftClickBlock event) {
		...
		if (player.motionY <= -0.1F) {
			if (player.getHeldItemMainhand().getItem() instanceof ItemHammer) {
				if (CropHelper.getWateringHandler(event.getWorld(), event.getPos(), event.getWorld().getBlockState(event.getPos())) !=
						null) {
					((ItemHammer) player.getHeldItemMainhand().getItem()).smashBlock(
							event.getWorld(),
							player,
							event.getPos(),
