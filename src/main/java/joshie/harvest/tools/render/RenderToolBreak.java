...
public class RenderToolBreak {
			...
			if (rayTraceResult != null) {
				if (isHammer) {
					...
					if (hammer.canUse(tool)) {
						...
						if (controller.getIsHittingBlock()) {
							drawBlockDamageTexture(
									MCClientHelper.getMinecraft().renderGlobal, controller,
					...
					if (hoe.canUse(tool)) {
						drawSelection(
								event.getContext(),
								hoe.getBlocks(world, rayTraceResult.getBlockPos(), player, tool),
								player,
	...
	private void drawBlockDamageTexture(
			RenderGlobal renderGlobal,
			PlayerControllerMP controllerMP,
