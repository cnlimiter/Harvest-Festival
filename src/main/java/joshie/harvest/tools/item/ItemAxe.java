...
public class ItemAxe extends ItemToolSmashing<ItemAxe> {
		...
		if (tag.hasKey("Block")) {
			...
			if (internal.equals(pos)) {
				...
				player.world.playSound(
						...
						(player.world.rand.nextFloat() * 0.25F) * times * 10F,
						player.world.rand.nextFloat() * 1.0F + 0.5F);
	...
	public float getDestroySpeed(@Nonnull ItemStack stack, IBlockState state) {
		if (canUse(stack)) {
			...
			return material != Material.WOOD && material != Material.PLANTS && material != Material.VINE ? super.getDestroySpeed(
					stack,
