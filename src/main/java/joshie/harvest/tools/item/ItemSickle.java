...
public class ItemSickle extends ItemToolChargeable<ItemSickle> {
	...
	public float getDestroySpeed(@Nonnull ItemStack stack, IBlockState state) {
		if (canUse(stack)) {
			...
			return (state.getBlock() != Blocks.GRASS && material == Material.GRASS) || material == Material.LEAVES ||
					material == Material.VINE ? 10F : super.getDestroySpeed(stack, state);
		} else {
			return 0.05F;
		}
	}
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(@Nonnull EntityEquipmentSlot slot, ItemStack stack) {
		...
		if (slot == EntityEquipmentSlot.MAINHAND) {
			multimap.put(
					...
					new AttributeModifier(ATTACK_DAMAGE_MODIFIER, "Weapon modifier", 5.0D, 0));
			multimap.put(
					SharedMonsterAttributes.ATTACK_SPEED.getName(),
					new AttributeModifier(ATTACK_SPEED_MODIFIER, "Weapon modifier", 3D + (tier.getToolLevel() - 6.0D), 0));
		}
	...
	protected String getLevelName(@Nonnull ItemStack stack, int charges) {
		...
		return charge == newCharge ? null : TextHelper.translate(
				"sickle.tooltip.charge." + LEVEL_TO_TIER.get(newCharge).name().toLowerCase(Locale.ENGLISH));
	}
}