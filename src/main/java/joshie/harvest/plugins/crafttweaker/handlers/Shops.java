...
public class Shops {
	...
	public static void addShopToNPC(String npc, String shop, String greeting, String openinghours, @Optional String hoursText) {
		...
		} else if (Shop.REGISTRY.containsKey(new ResourceLocation("MineTweaker3", shop.toLowerCase()))) {
			CraftTweaker.logError(String.format("Attempted to add a shop with a duplicate id: %s", "MineTweaker3:" + shop.toLowerCase()));
		} else {
			CraftTweakerAPI.apply(new AddShop(theNPC, shop, greeting, openinghours, hoursText));
		}
	}
	...
	private static class AddShop implements IAction {
		...
		AddShop(NPC npc, String shop, String greeting, String openinghours, @Nullable String hoursText) {
			...
			if (hoursText != null) {
				this.hours = new GreetingShopWrapper(hoursText);
			}
		}
	...
	public static void removePurchasable(String shop, String input) {
		...
		if (theShop == null) {
			...
			CraftTweakerAPI.apply(new RemovePurchasable(theShop, theShop.getPurchasableFromID(id)));
		}
	}
	...
	public static void adjustPurchasable(String shop, String input, long cost, @Optional IItemStack[] materials) {
		...
		if (theShop == null) {
			...
			CraftTweakerAPI.apply(new AdjustPurchasable(
					theShop,
					theShop.getPurchasableFromID(id),
					cost,
					CraftTweaker.asRequirements(materials)));
		}
	}
	...
	private static class AdjustPurchasable implements IAction {
		...
		public void apply() {
			...
			wrapper = required != null ? new PurchasableWrapperMaterials(purchasable, cost, required) : new PurchasableWrapper(
					purchasable,
					cost);
	...
	public static void adjustCarpenter(String input, int logs, int stone, long cost) {
		String id = fixPurchasableID(input);
		if (HFShops.CARPENTER.getPurchasableFromID(id) == null) {
			CraftTweaker.logError(String.format("No purchasable with the id %s could be found in " + HFShops.CARPENTER, id));
		} else if (!(HFShops.CARPENTER.getPurchasableFromID(id) instanceof PurchasableMaterials)) {
			CraftTweaker.logError(String.format("The item %s did not originally accept materials, you cannot adjust the values", id));
		} else {
			CraftTweakerAPI.apply(new AdjustCarpenter(HFShops.CARPENTER.getPurchasableFromID(id), cost, logs, stone));
		}
	}
	...
	private static String fixPurchasableID(String id) {
		if (!id.startsWith("buy") || !id.startsWith("sell")) {
			return "buy:" + id;
		} else {
			return id;
		}
