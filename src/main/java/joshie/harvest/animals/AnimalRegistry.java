...
public class AnimalRegistry implements IAnimalHandler {
	...
	public boolean canEat(@Nonnull ItemStack stack, AnimalFoodType... types) {
		for (AnimalFoodType type : types) {
			if (registry.matches(stack, type)) {
				return true;
			}
		}
	...
	public AnimalStats<NBTTagCompound> newStats(AnimalType type) {
		if (type == AnimalType.POULTRY) {
			return new AnimalStatsPoultry();
		} else if (type == AnimalType.LIVESTOCK) {
			return new AnimalStatsLivestock();
		} else if (type == AnimalType.MILKABLE) {
			return new AnimalStatsMilkable();
		} else {
			return new AnimalStatsHF();
		}
	}
	...
	public void syncAnimalStats(EntityAnimal animal) {
		...
		if (stats != null) {
			PacketHandler.sendToAllAround(
					new PacketSyncAnimal(animal.getEntityId(), stats),
					animal.dimension,
					animal.posX,
					animal.posY,
					animal.posZ,
