...
public class ModelHarvestSheep extends ModelBase {
	...
	public void render(
			...
			float scale) {
		if (isChild) {
			child.render(entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scale);
		} else {
			...
			if (sheep != null && sheep.getSheared()) {
				...
				wooly.render(entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scale);
			}
		}
	}
