...
public class SeasonData {
	...
	public float getCelestialAngle(long time) {
		time = HFCalendar.TICKS_PER_DAY == 24000 ? time + 6000 : (long) convertRange(0, HFCalendar.TICKS_PER_DAY - 1, 0, 23999, time) +
				6000;
		if (time < sunrise) {
			time += 24000;
		}
		if (time >= sunrise && time < midday) {
			return convertRange(sunrise, midday - 1, 0.757F, 1F, time);
		} else if (time >= midday && time < sunset) {
			return convertRange(midday, sunset - 1, 0F, 0.2425F, time);
		} else if (time >= sunset && time < midnight) {
			return convertRange(sunset, midnight - 1, 0.2425F, 0.5F, time);
		} else if (time >= midnight) {
			return convertRange(midnight, 24000 + sunrise, 0.5F, 0.757F, time);
		} else {
			return 1F;
		}
	}
