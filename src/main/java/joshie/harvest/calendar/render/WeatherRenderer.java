		...
		if (f > 0.0F) {
			...
			for (int k1 = k - i1; k1 <= k + i1; ++k1) {
				for (int l1 = i - i1; l1 <= i + i1; ++l1) {
					...
					double d3 = renderer.rainXCoords[i2] * 0.5D;
					double d4 = renderer.rainYCoords[i2] * 0.5D;
					...
					if (biome.canRain() || biome.getEnableSnow()) {
						...
						if (k2 != l2) {
							...
							if ((!weather.isSnow() && !biome.isSnowyBiome()) || biome.isHighHumidity()) {
								...
								double d5 = -(
										(double) (
												renderer.rendererUpdateCount + l1 * l1 * 3121 + l1 * 45238971 + k1 * k1 * 418711 +
														k1 * 13761 & 31) + (double) partialTicks) / 32.0D *
										(3.0D + renderer.random.nextDouble());
								...
								int k3 = j3 >> 16 & 65535;
								int l3 = j3 & 65535;
								vertexbuffer.pos(l1 - d3 + 0.5D, l2, k1 - d4 + 0.5D)
										.tex(0.0D, k2 * 0.25D + d5)
										.color(1.0F, 1.0F, 1.0F, f4)
										.lightmap(k3, l3)
										.endVertex();
								vertexbuffer.pos(l1 + d3 + 0.5D, l2, k1 + d4 + 0.5D)
										.tex(1.0D, k2 * 0.25D + d5)
										.color(1.0F, 1.0F, 1.0F, f4)
										.lightmap(k3, l3)
										.endVertex();
								vertexbuffer.pos(l1 + d3 + 0.5D, k2, k1 + d4 + 0.5D)
										.tex(1.0D, l2 * 0.25D + d5)
										.color(1.0F, 1.0F, 1.0F, f4)
										.lightmap(k3, l3)
										.endVertex();
								vertexbuffer.pos(l1 - d3 + 0.5D, k2, k1 - d4 + 0.5D)
										.tex(0.0D, l2 * 0.25D + d5)
										.color(1.0F, 1.0F, 1.0F, f4)
										.lightmap(k3, l3)
										.endVertex();
							} else {
								...
								int j4 = i4 >> 16 & 65535;
								int k4 = i4 & 65535;
								vertexbuffer.pos(l1 - d3 + 0.5D, l2, k1 - d4 + 0.5D).tex(0.0D + d9, k2 * 0.25D + d8 + d10).color(
										1.0F,
										1.0F,
										1.0F,
										f5).lightmap(j4, k4).endVertex();
								vertexbuffer.pos(l1 + d3 + 0.5D, l2, k1 + d4 + 0.5D).tex(1.0D + d9, k2 * 0.25D + d8 + d10).color(
										1.0F,
										1.0F,
										1.0F,
										f5).lightmap(j4, k4).endVertex();
								vertexbuffer.pos(l1 + d3 + 0.5D, k2, k1 + d4 + 0.5D).tex(1.0D + d9, l2 * 0.25D + d8 + d10).color(
										1.0F,
										1.0F,
										1.0F,
										f5).lightmap(j4, k4).endVertex();
								vertexbuffer.pos(l1 - d3 + 0.5D, k2, k1 - d4 + 0.5D).tex(0.0D + d9, l2 * 0.25D + d8 + d10).color(
										1.0F,
										1.0F,
										1.0F,
										f5).lightmap(j4, k4).endVertex();
