...
public class PlayerTrackerServer extends PlayerTracker implements ISyncMaster {
	...
	public void sync(EntityPlayer other, PacketSharedSync packet) {
		if (other != null) {
			PacketHandler.sendToClient(packet, other);
		} else {
	...
	public void newDay(CalendarDate yesterday, CalendarDate today) {
		...
		if (player != null) {
			...
			if (gold > 0) {
				EventTrigger.INSTANCE.trigger(player, "first_shipping");
			}
			stats.addGold(null, gold);
			if (stats.getGold() >= 1000000) {
				EventTrigger.INSTANCE.trigger(player, "millionaire");
			}
			...
			if (yearsPassed >= 2) {
				if (today.getSeason() == Season.SPRING) {
					EventTrigger.INSTANCE.trigger(player, "strawberry");
				} else if (today.getSeason() == Season.SUMMER) {
					EventTrigger.INSTANCE.trigger(player, "corn");
				} else if (today.getSeason() == Season.AUTUMN) {
					EventTrigger.INSTANCE.trigger(player, "green_pepper");
				}
			} else if (yearsPassed >= 1) {
				if (today.getSeason() == Season.SPRING) {
					EventTrigger.INSTANCE.trigger(player, "cucumber");
				} else if (today.getSeason() == Season.SUMMER) {
					EventTrigger.INSTANCE.trigger(player, "tomato");
				} else if (today.getSeason() == Season.AUTUMN) {
					EventTrigger.INSTANCE.trigger(player, "eggplant");
				}
			}
		}
	}
