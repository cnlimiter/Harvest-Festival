...
public class TrackingServer extends Tracking {
	...
	public long newDay() {
		...
		if (HFCore.DISPLAY_SHIPPED_LIST && !toBeShipped.isEmpty()) {
	...
	private void addDefaultRecipes() {
		...
		Recipe.REGISTRY.values().stream().filter(recipe -> recipe.isDefault() && !recipes.contains(recipe.getResource())).forEachOrdered(
				recipe -> recipes.add(recipe.getResource()));
	}
