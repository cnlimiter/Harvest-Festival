...
public class NPCHolidayStoreSpecial extends NPCSpecialSeller {
	...
	public Shop getShop(World world, BlockPos pos, @Nullable EntityPlayer player) {
		Festival festival = player != null ?
				TownHelper.getClosestTownToEntity(player, false).getFestival() :
				TownHelper.getClosestTownToBlockPos(world, pos, false).getFestival();
		if (shops.containsKey(festival)) {
			return shops.get(festival);
		} else {
