...
public class GuiNPCGift extends GuiNPCChat {
	...
	public String getScript() {
		if (NPCHelper.INSTANCE.getGifts().isBlacklisted(player.world, player, gift)) {
			return TextHelper.getSpeech(npc, "gift.no");
		}
		...
		if (GODDESS_GIFT != null || HFTrackers.getClientPlayerTracker().getRelationships().gift(
				player,
				npc.getNPC(),
				value.getRelationPoints())) {
			return TextHelper.getSpeech(npc, "gift." + value.name().toLowerCase(Locale.ENGLISH));
		} else {
			return TextHelper.getSpeech(npc, "gift.reject");
		}
	}
