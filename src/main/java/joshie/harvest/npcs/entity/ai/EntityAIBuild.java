...
public class EntityAIBuild extends EntityAIBase {
	...
	private void attemptToTeleportToSafety(BlockPos go) {
		Vec3d vec = RandomPositionGenerator.findRandomTargetBlockTowards(
				...
				3,
				32,
				new Vec3d(go.getX() + 0.5D, go.getY() + 1D, go.getZ() + 0.5D));
		if (vec != null) {
			BlockPos pos = new BlockPos(vec);
			if (EntityHelper.isSpawnable(npc.world, pos) && EntityHelper.isSpawnable(npc.world, pos.up()) &&
					npc.world.getBlockState(pos.down()).isSideSolid(npc.world, pos.down(), EnumFacing.UP)) {
