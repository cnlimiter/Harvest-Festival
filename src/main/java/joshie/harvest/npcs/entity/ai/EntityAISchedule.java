...
public class EntityAISchedule extends EntityAIBase {
	...
	private BuildingLocation getBuildingTarget(CalendarDate date) {
		return npc.getNPC().getScheduler().getTarget(
				npc.world,
				npc,
				date.getSeason(),
				date.getWeekday(),
				CalendarHelper.getTime(npc.world));
	}
	...
	private Path getPathToTarget() {
		...
		if (path != null) {
			...
			Vec3d vec = RandomPositionGenerator.findRandomTargetBlockTowards(
					...
					16,
					7,
					new Vec3d(blockTarget.getX() + 0.5D, blockTarget.getY(), blockTarget.getZ() + 0.5D));
			if (vec != null) {
	...
	private Path getPathAwayFromTarget() {
		Vec3d vec = RandomPositionGenerator.findRandomTargetBlockAwayFrom(
				npc,
				(int) distanceRequired / 2,
				3,
				new Vec3d(blockTarget.getX() + 0.5D, blockTarget.getY(), blockTarget.getZ() + 0.5D));
		if (vec != null) {
	...
	private boolean shouldTeleport() {
		if (!blockTarget.equals(prevTarget)) {
			teleportTimer = 0;
		} else {
			teleportTimer++; //Update the teleport timer if the target is the same
		}
