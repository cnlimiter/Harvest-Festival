...
public class EntityNPCGoddess extends EntityNPC<EntityNPCGoddess> {
	...
	public void travel(float strafe, float vertical, float forward) {
		if (isInWater()) {
			...
			if (onGround) {
				f = world.getBlockState(new BlockPos(
						MathHelper.floor(posX),
						MathHelper.floor(getEntityBoundingBox().minY) - 1,
						MathHelper.floor(posZ))).getBlock().slipperiness * 0.91F;
			}
	...
	public void onLivingUpdate() {
		...
		for (int i = 0; i < 16; i++) {
			world.spawnParticle(
					...
					posX + 0.1 * rand.nextFloat(),
					posY + 0.2 * rand.nextFloat(),
					posZ + 0.1 * rand.nextFloat(),
					0,
					-0.05,
					0);
		}
	...
	public void readSpawnData(ByteBuf buf) {
		...
		for (int i = 0; i < 16; i++) {
			world.spawnParticle(
					...
					posX * 0.2 * rand.nextFloat(),
					posY + 0.5 + 0.2 * rand.nextFloat(),
					posZ + 0.2 * rand.nextFloat(),
					0,
					0,
					0);
		}
