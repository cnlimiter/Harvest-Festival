...
public class QuestBlacksmithing extends QuestTrade {
	...
	public boolean isNPCUsed(EntityPlayer player, NPCEntity entity) {
		return entity.getNPC() == HFNPCs.BLACKSMITH &&
				(isHolding(player) != null || isHoldingBrokenTool(player) != null || !tool.isEmpty());
	}
	...
	public String getLocalizedScript(EntityPlayer player, NPCEntity entity) {
		if (quest_stage == TEST) {
			if (today.getWeekday() == Weekday.THURSDAY || daytime < 10000 || daytime > 16000) {
				return getLocalized("closed.start");
			}
			//Repairing
			ToolTier broken = isHoldingBrokenTool(player);
			if (broken != null) {
				long required = getCost(broken) / 10;
				if (HFTrackers.getPlayerTrackerFromPlayer(player).getStats().getGold() < required) {
			...
			if (holding != null) {
				...
				if (getLevel(player) != 100D) {
					return getLocalized("level");
				}
				ItemTool itemTool = (ItemTool) player.getHeldItemMainhand().getItem();
				if (!HFTools.TOOLTYPE_MAP.containsKey(itemTool.toolClass)) {
					return getLocalized("repair.no");
				}
				...
				if (!hasGold) {
					return getLocalized("gold", required);
				}
			...
			if (getDifference(date, today) >= days) {
				if (today.getWeekday() == Weekday.THURSDAY || daytime < 10000 || daytime > 16000) {
					return getLocalized("closed.finish", tool.getDisplayName());
				}
			...
			if (getDifference(date, today) >= days) {
				...
				for (int i = 0; i < 32; i++) {
					player.world.spawnParticle(
							...
							living.posX + player.world.rand.nextFloat() + player.world.rand.nextFloat() - 1F,
							living.posY + 0.25D + living.getEntityWorld().rand.nextFloat() + living.getEntityWorld().rand.nextFloat(),
							living.posZ + player.world.rand.nextFloat() + player.world.rand.nextFloat() - 1F,
							0,
							0,
							0);
				}
