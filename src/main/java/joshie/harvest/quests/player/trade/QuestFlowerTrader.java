	...
	public String getLocalizedScript(EntityPlayer player, NPCEntity entity) {
		...
		if (received < 10) {
			...
			if (season == SPRING || season == SUMMER || season == AUTUMN) {
				...
				return chance == 0 ? getLocalized("complete.yes1") : chance == 1 ? getLocalized("complete.yes2") : getLocalized(
						"complete.yes3");
				//Jade Informs the player that she doesn't have in stock this time of year
			} else {
				return getLocalized("complete.no");
			}
		} else {
			return getLocalized("return", 5 - days);
		}
	}
