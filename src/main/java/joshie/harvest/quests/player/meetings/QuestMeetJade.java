...
public class QuestMeetJade extends QuestQuestion {
	...
	public Selection getSelection(EntityPlayer player, NPCEntity entity) {
		if (!entity.getTown().hasBuilding(HFBuildings.CARPENTER) || isCompletedEarly()) {
			return null;
		}
	...
	public String getTitle() {
		if (quest_stage == INTRO) {
			return I18n.translateToLocal(
					getRegistryName().getResourceDomain() + ".quest." + getRegistryName().getResourcePath() + ".title.what");
		} else {
			return I18n.translateToLocal(
					getRegistryName().getResourceDomain() + ".quest." + getRegistryName().getResourcePath() + ".title");
		}
	}
	...
	public String getDescription(World world, EntityPlayer player) {
		if (quest_stage == INTRO) {
			...
			return getLocalized("description.turnips");
		} else {
