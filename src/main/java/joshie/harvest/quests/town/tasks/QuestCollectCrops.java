...
public class QuestCollectCrops extends QuestDaily {
	...
	public String getDescription(World world, @Nullable EntityPlayer player) {
		if (player != null) {
			return getLocalized("desc", amount, crop.getLocalizedName(true));
		} else {
	...
	public boolean isNPCUsed(EntityPlayer player, NPCEntity entity) {
		if (!super.isNPCUsed(player, entity)) {
			return false;
		}
	...
	public void onQuestCompleted(EntityPlayer player) {
		...
		ItemStack stack = HFCooking.MEAL.getCreativeStack(ItemMeal.MEALS[player.world.rand.nextInt(Meal.values().length)]);
		stack.setCount(3);
