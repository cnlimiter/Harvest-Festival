	...
	public String getLocalized(NPCEntity entity) {
		...
		if (quest != null) {
			...
			if (entry != null && !entry.isInvalid(entity.getAsEntity().getEntityWorld())) {
				return TextHelper.format(
						unlocalised,
						entry.getName(entity.getAsEntity().getEntityWorld()),
						entry.getTextFromScore(unlocalised, entry.getScore(quest, entity.getAsEntity().getEntityWorld())));
			}
		}
