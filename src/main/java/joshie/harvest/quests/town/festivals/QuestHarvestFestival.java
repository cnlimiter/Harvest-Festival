...
public class QuestHarvestFestival extends QuestFestivalTimed {
	...
	private static final BlockPos[] NEAR_POT = new BlockPos[]{
			new BlockPos(1, 2, 14),
			new BlockPos(1, 2, 12),
			new BlockPos(2, 2, 13),
			new BlockPos(0, 2, 13)};
	...
	protected String getLocalizedScript(EntityPlayer player, NPC npc) {
		if (!getDataForPlayer(player).hasStarted()) {
			return getLocalized("welcome");
		} else {
			return getLocalized("started");
		}
	}
	...
	public void execute(EntityPlayer player, NPCEntity entity) {
		...
		entity.setPath(
				TaskMove.of(TownHelper.getClosestTownToEntity(player, false)
						.getCoordinatesFromOffset(HFBuildings.FESTIVAL_GROUNDS, BESIDE_POT)),
	...
	public static int getPotScore(NPCEntity npc) {
		BlockPos pos = TownHelper.getClosestTownToEntity(npc.getAsEntity(), false).getCoordinatesFromOffset(
			...
			if (quest != null) {
				...
				for (UUID uuid : quest.data.keySet()) {
					for (NPC inhabitant : data.getInhabitants()) {
						HFTrackers.getPlayerTracker(npc.getAsEntity().world, uuid).getRelationships().affectRelationship(
								inhabitant,
								relationship);
					}
				}
