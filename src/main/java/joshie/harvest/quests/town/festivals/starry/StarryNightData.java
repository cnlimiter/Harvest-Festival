...
public class StarryNightData extends Selection<QuestStarryNight> {
	...
	private static final BlockPos[] plates = new BlockPos[]{
			new BlockPos(10, 2, 13),
			new BlockPos(10, 2, 14),
	...
	private static final String[] lines2 = new String[]{
			...
			"harvestfestival.quest.festival.starry.night.go",
			"harvestfestival.quest.festival.starry.night.no"};
	...
	public StarryNightData() {
		super(
				"harvestfestival.quest.festival.starry.night.what",
				"harvestfestival.quest.festival.starry.night.invite",
				"harvestfestival.quest.festival.starry.night.chat");
	}
	...
	public String getLocalized(String quest, Object... format) {
		if (format.length == 0) {
			return I18n.translateToLocal(prefix + "." + quest.replace("_", ""));
		} else {
			return I18n.translateToLocalFormatted(prefix + "." + quest.replace("_", ""), format);
		}
	}
	...
	public String getLocalizedScript(NPC npc) {
		...
		} else if (!completed && invited == npc) {
			return TextHelper.getSpeech(npc, "festival.starry.night.start");
		} else {
			return invited == npc ? TextHelper.getSpeech(npc, "festival.starry.night.tonight") : null;
		}
	}
	...
	private void startSequence(EntityPlayer player, NPCEntity entity) {
		...
		if (playerPlate != null) {
			positions.add(playerPlate);
		}
		if (npcPlate != null) {
			positions.add(npcPlate);
		}
	...
	public static class ConsumeFood extends TaskElement {
		...
		public void execute(NPCEntity npc) {
			...
			for (BlockPos pos : positions) {
				if (destroy) {
					npc.getAsEntity().world.setBlockToAir(pos);
				} else {
	...
	public static StarryNightData fromNBT(NBTTagCompound tag) {
		...
		if (tag.hasKey("NPC")) {
			data.invited = NPC.REGISTRY.get(new ResourceLocation(tag.getString("NPC")));
		}
		return data;
	}
	...
	public NBTTagCompound toNBT() {
		...
		if (invited != null) {
			tag.setString("NPC", invited.getResource().toString());
		}
		return tag;
	}
}
