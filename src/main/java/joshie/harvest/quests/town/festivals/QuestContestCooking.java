...
public class QuestContestCooking extends QuestContest<CookingContestEntries> {
	...
	private static final BlockPos[] LOCATIONS = new BlockPos[]{
			new BlockPos(8, 3, 22),
			new BlockPos(9, 3, 22),
			new BlockPos(10, 3, 22),
			new BlockPos(11, 3, 22)};
	private static final NPC[] NPCS = new NPC[]{
			HFNPCs.FLOWER_GIRL,
			HFNPCs.MILKMAID,
			HFNPCs.TRADER,
			HFNPCs.CARPENTER,
			HFNPCs.DAUGHTER_ADULT,
			HFNPCs.CLOCKMAKER,
			HFNPCs.BLACKSMITH,
			HFNPCs.FISHERMAN,
			HFNPCs.POULTRY,
	...
	public String getLocalizedScript(EntityPlayer player, NPCEntity entity) {
		...
		if (quest_stage == CONTINUE) {
			return getLocalized("judging");
		}
	...
	public void execute(Town town, EntityPlayer player, NPCEntity npc) {
		npc.setPath(
				move(STAND1),
				new TaskEat(town, LOCATIONS[0]),
				speech(JUDGE_1),
				move(STAND2),
				new TaskEat(town, LOCATIONS[1]),
				speech(JUDGE_2),
				move(STAND3),
				new TaskEat(town, LOCATIONS[2]),
				speech(JUDGE_3),
				move(STAND4),
				new TaskEat(town, LOCATIONS[3]),
				speech(JUDGE_4),
				wait(1),
				speech(FINISH),
