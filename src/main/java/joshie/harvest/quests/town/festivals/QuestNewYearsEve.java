	...
	private final Selection start = new Selection(
		...
		public Result onSelected(EntityPlayer player, NPCEntity entity, @Nullable Quest quest, int option) {
			if (option == 1) {
				...
				entity.setPath(
						new TaskEat(town, new BlockPos(13, 2, 16)),
						new TaskEat(town, new BlockPos(13, 2, 14)),
						new TaskEat(town, new BlockPos(13, 2, 12)),
						TaskSpeech.of(scriptIntro),
						TaskWait.of(1),
						TaskSpeech.of(scriptCountdown5),
						TaskWait.of(1),
						TaskSpeech.of(scriptCountdown4),
						TaskWait.of(1),
						TaskSpeech.of(scriptCountdown3),
						TaskWait.of(1),
						TaskSpeech.of(scriptCountdown2),
						TaskWait.of(1),
						TaskSpeech.of(scriptCountdown1),
						TaskWait.of(1),
						new TaskNewYear(),
						TaskSpeech.of(scriptCountdown0));
			}
	...
	public static class TaskNewYear extends TaskElement {
		...
		public void execute(NPCEntity entity) {
			...
			for (EntityPlayer player : EntityHelper.getEntities(EntityPlayer.class, npc.world, new BlockPos(npc), 64, 64)) {
				NPC.REGISTRY.values().stream().filter(aNPC -> HFApi.player.getRelationsForPlayer(player)
								.isStatusMet(aNPC, RelationStatus.MET))
		...
		public static void startFireworksShow(EntityLiving npc, Town town) {
			int[] colors = {
					...
					15435844,
					15790320};
			BlockPos[] locations = new BlockPos[]{
					new BlockPos(8, 1, 11), new BlockPos(8, 1, 15), new BlockPos(8, 1, 19), new BlockPos(
					8,
					1,
					23)};
		...
		private static void summonFireworks(
				EntityLiving npc,
				Town town,
				BlockPos pos,
				int lifetime,
