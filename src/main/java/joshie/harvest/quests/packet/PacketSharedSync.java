...
public class PacketSharedSync extends PenguinPacket {
	...
	public <Q extends QuestData> Q getQuestDataFromPlayer(EntityPlayer player) {
		if (uuid == null) {
			...
			return (Q) HFTrackers.getTowns(player.world).getTownByID(uuid).getQuests();
		}
	}
	...
	public <L extends LetterData> L getLetterDataFromPlayer(EntityPlayer player) {
		if (uuid == null) {
			return (L) HFTrackers.getPlayerTrackerFromPlayer(player).getLetters();
		} else {
			return (L) HFTrackers.getTowns(player.world).getTownByID(uuid).getLetters();
		}
	}
}