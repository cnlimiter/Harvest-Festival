...
public class WateringHandler {
	...
	public void dehydrate(World world, BlockPos pos, IBlockState state, boolean forceDirt) {
		...
		if (!forceDirt && moisture == WET_SOIL) {
			world.setBlockState(pos, state.withProperty(BlockFarmland.MOISTURE, DRYING_SOIL), 2);
		} else if (!forceDirt && moisture == DRYING_SOIL) {
			world.setBlockState(pos, state.withProperty(BlockFarmland.MOISTURE, DRY_SOIL), 2);
