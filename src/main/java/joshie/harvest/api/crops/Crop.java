...
public class Crop extends HFRegistry<Crop> implements IPlantable {
	...
	public Crop setIngredient(int hunger, float saturation) {
		...
		if (Ingredient.INGREDIENTS.containsKey(name)) {
			...
			this.ingredient = new Ingredient(name, hunger, saturation);
		}
	...
	public String getLocalizedName(boolean isItem) {
		...
		return I18n.translateToLocal((
				getResource().getResourceDomain() + ".crop." + StringUtils.replace(getResource().getResourcePath(), "_", ".") + suffix));
	}
	...
	public String getSeedsName() {
		String name = alternativeName ?
				I18n.translateToLocalFormatted((
						getResource().getResourceDomain() + ".crop." + StringUtils.replace(getResource().getResourcePath(), "_", ".") +
								".block")) :
				item.isEmpty() ? "NULL" : item.getDisplayName();
