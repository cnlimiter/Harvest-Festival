...
public class StateHandlerTree extends StateHandlerDefault<Tree> {
	private static final AxisAlignedBB SAPLING_AABB = new AxisAlignedBB(
			...
			0.800000011920929D,
			0.8999999761581421D);
	private static final AxisAlignedBB JUVENILE_UPPER_AABB = new AxisAlignedBB(
			0.09999999403953552D,
			-1.0D,
			0.09999999403953552D,
			0.8999999761581421D,
			0.800000011920929D,
			0.8999999761581421D);
	private static final AxisAlignedBB JUVENILE_LOWER_AABB = new AxisAlignedBB(
			0.09999999403953552D,
			0.0D,
			0.09999999403953552D,
			0.8999999761581421D,
	...
	public IBlockState getState(IBlockAccess world, BlockPos pos, PlantSection section, Tree tree, int stage, boolean withered) {
		if (section == PlantSection.TOP) {
			return stage < stage3 ? getState(4) : log;
