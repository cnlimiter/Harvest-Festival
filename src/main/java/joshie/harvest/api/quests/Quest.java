...
public abstract class Quest extends IForgeRegistryEntry.Impl<Quest> {
	 ...
	 **/
	public static final ForgeRegistry<Quest> REGISTRY = (ForgeRegistry<Quest>) new RegistryBuilder<Quest>().setName(new ResourceLocation(
			"harvestfestival",
			"quests")).setType(Quest.class).setIDRange(0, 32000).create();
	...
	public String getLocalized(String quest, Object... format) {
		if (format.length == 0) {
			return I18n.translateToLocal(getPrefix() + "." + quest.replace("_", ""));
		} else {
			return I18n.translateToLocalFormatted(getPrefix() + "." + quest.replace("_", ""), format);
		}
	}
