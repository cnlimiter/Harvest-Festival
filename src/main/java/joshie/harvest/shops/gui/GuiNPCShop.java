...
public class GuiNPCShop extends GuiNPCBase {
	...
	public GuiNPCShop(EntityPlayer player, EntityNPC npc, int nextGui, boolean isSelling) {
		...
		if (shop == null || !NPCHelper.isShopOpen(npc.world, npc, player, npc.getNPC().getShop(npc.world, pos, player))) {
			player.closeScreen();
		}
	...
	public void setStart(int i) {
		...
		while (it.hasNext() && position <= 180) {
			...
			if (pPosition >= start && purchasable.canList(client.world, client) && data.canList(shop, purchasable)) {
				if (purchasable.getCost() < 0) {
					...
					position += addButton(purchasable, id + 2, guiLeft + 28, 38 + guiTop + position, position);
				}
		...
		if (buttonList.size() == 2) {
			buttonList.add(new ButtonListingOutOfStock(this, 3, guiLeft + 28, 38 + guiTop + position));
		}
	}
	...
	public void updatePurchased(@Nonnull ItemStack stack, int amount) {
		if (stack.isEmpty()) {
			purchased = ItemStack.EMPTY;
		} else if (purchased.isEmpty() || !(
				ItemStack.areItemsEqual(purchased, stack) && ItemStack.areItemStackTagsEqual(
						purchased,
						stack))) {
			purchased = stack.copy();
		} else {
			purchased.setCount(purchased.getCount() + amount);
		}
	}
	...
	protected void drawTooltip(List<String> list, int x, int y) {
		if (purchased.isEmpty()) {
			super.drawTooltip(list, x, y);
		} else {
			super.drawTooltip(list, x + 16, y + 16);
		}
	}
	...
	private void drawPlayerInventory() {
		if (selling) {
			ShopFontRenderer.render(this, guiLeft + 250, guiTop + 27, "SELLING", false);
		} else {
			ShopFontRenderer.render(this, guiLeft + 250, guiTop + 27, "BUYING", false);
		}
