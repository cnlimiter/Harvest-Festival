...
public class ButtonListing<I extends IPurchasable> extends GuiButton {
		...
		if (clicked && hovered) {
			if (hoverTimer == 0 || (hoverTimer % 20 == 0 && GuiScreen.isCtrlKeyDown())) {
				if (GuiScreen.isShiftKeyDown() && canPurchaseX(stackSize + 10)) {
					...
					if (purchasable.getCost() >= 0) {
						shop.updatePurchased(
								StackHelper.toStack(
										purchasable.getDisplayStack(),
										10 * purchasable.getDisplayStack().getCount()), 10 * purchasable.getDisplayStack().getCount());
					}
					if (hoverTimer % 40 == 1) {
					...
					stackSize++;
					if (purchasable.getCost() >= 0) {
						shop.updatePurchased(purchasable.getDisplayStack(), purchasable.getDisplayStack().getCount());
					}
					StatsClient stats = HFTrackers.getClientPlayerTracker().getStats();
	...
	private boolean canPurchaseX(int x) {
		StatsClient stats = HFTrackers.getClientPlayerTracker().getStats();
		return (purchasable.getStock() == 0 || x <= purchasable.getStock()) && stats.getGold() - (purchasable.getCost() * x) >= 0 &&
				purchasable.canDo(MCClientHelper.getWorld(), MCClientHelper.getPlayer(), x);
	}
