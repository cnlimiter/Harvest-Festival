...
public class Purchasable implements IPurchasable {
	...
	private static String stackToString(@Nonnull ItemStack stack) {
		...
		String string = String.valueOf(stack.getItem().getRegistryName()).replace(":", "_");
		if (stack.getItemDamage() != 0) {
			string = string + "_" + stack.getItemDamage();
		}
		if (stack.getTagCompound() != null) {
			string = string + "_" + stack.getTagCompound().hashCode();
		}
		return string;
	}
	...
	public void onPurchased(EntityPlayer player) {
		...
		if (note != null) {
			HFApi.player.getTrackingForPlayer(player).learnNote(note);
		}
	}
