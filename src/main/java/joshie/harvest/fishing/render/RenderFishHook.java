	...
	public void doRender(EntityFishHook entity, double x, double y, double z, float entityYaw, float partialTicks) {
		...
		if (entityplayer != null && !this.renderOutlines) {
			...
			GlStateManager.rotate(
					(this.renderManager.options.thirdPersonView == 2 ? -1 : 1) * -this.renderManager.playerViewX,
					1.0F,
					0.0F,
					0.0F);
			...
			float f9 =
					(entityplayer.prevRenderYawOffset + (entityplayer.renderYawOffset - entityplayer.prevRenderYawOffset) * partialTicks) *
							0.017453292F;
			...
			if ((this.renderManager.options == null || this.renderManager.options.thirdPersonView <= 0) &&
				...
				vec3d = vec3d.rotateYaw(
						-(entityplayer.prevRotationYaw + (entityplayer.rotationYaw - entityplayer.prevRotationYaw) * partialTicks) *
								0.017453292F);
				...
				d5 = entityplayer.prevPosY + entityplayer.getEyeHeight() + (entityplayer.posY - entityplayer.prevPosY) * partialTicks -
						0.45D;
