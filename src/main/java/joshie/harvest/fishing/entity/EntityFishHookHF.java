...
public class EntityFishHookHF extends EntityFishHook {
	...
	public int handleHookRetraction() {
		...
		if (!world.isRemote && angler != null) {
			...
			if (caughtEntity != null) {
				...
				builder.withLuck(this.luck + angler.getLuck());
				builder.withLootedEntity(this);
				builder.withPlayer(angler);
				...
				for (ItemStack stack : result) {
					...
					angler.world.spawnEntity(new EntityXPOrb(
							angler.world,
							angler.posX,
							angler.posY + 0.5D,
							angler.posZ + 0.5D,
	...
	public void catchingFish(BlockPos pos) {
		...
		if (ticksCatchable > 0 || ticksCatchableDelay > 0 || ticksCaughtDelay > 0) {
			return;
		}
