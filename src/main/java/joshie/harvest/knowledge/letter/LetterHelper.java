...
public class LetterHelper {
	...
	public static void addLetterToMailbox(EntityPlayer player, Letter letter) {
		if (!letter.isTownLetter()) {
			HFTrackers.<PlayerTrackerServer>getPlayerTrackerFromPlayer(player).getLetters().addLetterAndSync(letter);
		} else {
			TownHelper.<TownDataServer>getClosestTownToEntity(player, false).getLetters().addLetterAndSync(letter);
		}
