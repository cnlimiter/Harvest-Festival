...
public class ButtonNote extends ButtonBook {
	...
	public void drawButton(@Nonnull Minecraft mc, int mouseX, int mouseY, float partialTicks) {
		if (visible) {
			...
			if (PageNotes.note.equals(note)) {
				drawRect(x, y, x + width, y + height, 0x559C8C63);
			} else if (!hovered || !unlocked) {
				drawRect(x, y, x + width, y + height, 0x55B0A483);
			} else {
				drawRect(x, y, x + width, y + height, 0x55C4B9A2);
			}
			drawForeground(mc);
	...
	private void drawForeground(@Nonnull Minecraft mc) {
		if (note.getGuiResource() != null) {
			...
			if (unlocked) {
				gui.drawTexturedModalRect(x, y, note.getGuiX(), note.getGuiY(), 16, 16);
			} else {
				gui.drawTexturedModalRect(x, y, note.getGuiX() + 16, note.getGuiY(), 16, 16);
			}
			...
			if (unlocked) {
				StackRenderHelper.drawStack(note.getIcon(), x, y, 1F);
			} else {
				StackRenderHelper.drawGreyStack(note.getIcon(), x, y, 1F);
			}
		}
