...
public class ButtonRelationsAnimal extends ButtonBook<GuiStats> {
	...
	private void drawForeground() {
		...
		if (needsCleaning) {
			drawStack(isClean, HFAnimals.TOOLS.getStackFromEnum(Tool.BRUSH), 85, -5);
		}
	...
	private void drawStack(boolean value, @Nonnull ItemStack icon, int x, int y, float scale) {
		if (!value) {
			StackRenderHelper.drawGreyStack(icon, this.x + x, this.y + y, scale);
		} else {
			StackRenderHelper.drawStack(icon, this.x + x, this.y + y, scale);
		}
	}
