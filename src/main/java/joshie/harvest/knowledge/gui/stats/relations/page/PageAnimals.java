...
public class PageAnimals extends PageRelationship {
	...
	public void initGui(GuiStats gui, List<GuiButton> buttonList, List<GuiLabel> labelList) {
		...
		//Build the list
		List<EntityAnimal> list = EntityHelper.getEntities(
				EntityAnimal.class,
				MCClientHelper.getWorld(),
				new BlockPos(MCClientHelper.getPlayer()),
				128D,
				128D);
		...
		for (int i = start * 12; i < 12 + start * 12 && i < list.size(); i++) {
			...
			buttonList.add(new ButtonRelationsAnimal(
					gui,
					animal,
					EntityHelper.getStats(animal),
					buttonList.size(),
					16 + x * 144,
					26 + y * 25));
			y++;
		...
		if (buttonList.size() == 2) {
			buttonList.add(new ButtonRelationsAnimalNull(gui, buttonList.size(), 16 + x * 144, 26 + y * 25));
		}
