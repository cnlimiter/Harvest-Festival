...
public class PageNotes extends BookPage<GuiStats> {
	...
	public void initGui(GuiStats gui, List<GuiButton> buttonList, List<GuiLabel> labelList) {
		...
		list.removeIf(note -> note.isSecret() &&
				!HFTrackers.getClientPlayerTracker().getTracking().getLearntNotes().contains(note.getResource()));
		...
		if (start < maxStart) {
			buttonList.add(new ButtonNext(gui, buttonList.size(), 130, 172));
		}
		if (start != 0) {
		...
		if (note.getRender() != null) {
			note.getRender().initRender(gui.mc, gui, gui.guiLeft, gui.guiTop);
		}
	}
