...
public class BlockCookware extends BlockHFEnumRotatableTile<BlockCookware, Cookware> {
	...
	public boolean canRenderInLayer(IBlockState state, @Nonnull BlockRenderLayer layer) {
		...
		return cookware != Cookware.FRIDGE_TOP &&
				(cookware == Cookware.MIXER ? layer == BlockRenderLayer.TRANSLUCENT : layer == BlockRenderLayer.CUTOUT_MIPPED);
	}
		...
		if (getEnumFromState(state) == Cookware.FRIDGE) {
			...
			return !(
					world.getBlockState(pos.offset(side)).doesSideBlockRendering(world, pos.offset(side), side.getOpposite()) &&
							world.getBlockState(pos.offset(side).up()).doesSideBlockRendering(
									world,
									pos.offset(side).up(),
									side.getOpposite()));
		} else {
	...
	public IBlockState getActualState(@Nonnull IBlockState state, IBlockAccess world, BlockPos pos) {
		...
		if (cookware == Cookware.OVEN_OFF || cookware == Cookware.OVEN_ON) {
			TileEntity tile = world instanceof ChunkCache ?
					((ChunkCache) world).getTileEntity(pos.up(), Chunk.EnumCreateEntityType.CHECK) :
