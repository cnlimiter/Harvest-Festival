...
public class HFIngredients {
	...
	public static final Ingredient OIL = new Ingredient("oil", 0, 0.2F).setSellValue(ItemIngredients.Ingredient.OIL.getCost()).setFluid(
			IngredientMappingEvent.OIL);
	public static final Ingredient RICEBALL = new Ingredient(
			"riceball",
			1,
			0.25F).setSellValue(ItemIngredients.Ingredient.RICEBALL.getCost());
	public static final Ingredient CURRY_POWDER = new Ingredient(
			"curry_powder",
			1,
			0.2F).setSellValue(ItemIngredients.Ingredient.CURRY_POWDER.getCost());
	...
	public static final Ingredient WINE = new Ingredient("wine", 2, 0.8F).setSellValue(ItemIngredients.Ingredient.WINE.getCost()).setFluid(
			IngredientMappingEvent.WINE);
	...
	public static void postInit() {
		...
		Crop.REGISTRY.values().stream().filter(crop -> crop != Crop.NULL_CROP && crop.getIngredient() != null).forEachOrdered(crop -> {
			...
			if (name != null) {
				...
				HFApi.cooking.register(stack, crop.getIngredient());
			}
		});
