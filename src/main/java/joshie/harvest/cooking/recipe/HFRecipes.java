...
import static joshie.harvest.cooking.recipe.RecipeHelper.addFryingPanRecipe;
import static joshie.harvest.cooking.recipe.RecipeHelper.addMixerRecipe;
import static joshie.harvest.cooking.recipe.RecipeHelper.addNoUtensilRecipe;
import static joshie.harvest.cooking.recipe.RecipeHelper.addOvenRecipe;
import static joshie.harvest.cooking.recipe.RecipeHelper.addPotRecipe;
...
public class HFRecipes {
	...
	private static void addFryingPanRecipes() {
		...
		addFryingPanRecipe(Meal.OMELET_RICE, 1.15F, 1.1F, EGG, MILK, OIL, RICEBALL).setOptionalIngredients(
				CABBAGE,
				ONION,
				MUSHROOM,
				GREEN_PEPPER,
				SALT); //Brandon 10000RP
		...
		//Added in 0.6+
		addFryingPanRecipe(Meal.STIR_FRY, 1.2F, 1F, CABBAGE, OIL).setOptionalIngredients(
				ONION,
				CABBAGE,
				BAMBOO,
				MATSUTAKE,
				EGGPLANT,
				GREEN_PEPPER); //Jenni 20000RP
		addFryingPanRecipe(Meal.RICE_FRIED, 1.5F, 1F, RICEBALL, OIL, EGG).setOptionalIngredients(
				CARROT,
				ONION,
				BAMBOO,
				GREEN_PEPPER,
		...
		addFryingPanRecipe(Meal.NOODLES_THICK_FRIED, 1.2F, 1F, NOODLES, OIL).setOptionalIngredients(
				CABBAGE,
				ONION,
				FISH,
				BAMBOO,
				CARROT,
				EGGPLANT); //Cloe 20000RP
		addFryingPanRecipe(Meal.TEMPURA, 1.2F, 1F, EGG, FLOUR, OIL).setOptionalIngredients(
				EGGPLANT,
				GREEN_PEPPER,
				CARROT,
				CABBAGE,
				ONION); //Liara 17500RP
		addFryingPanRecipe(Meal.CURRY_DRY, 1.2F, 1F, RICEBALL, CURRY_POWDER).setOptionalIngredients(
				ONION,
				GREEN_PEPPER,
				FISH,
				POTATO,
				EGGPLANT,
				CARROT);//Ashlee 20000RP
	}
	...
	private static void addMixerRecipes() {
		...
		addMixerRecipe(Meal.JUICE_VEGETABLE, 1.5F, 0.6F, VEGETABLE_JUICE_BASE).setOptionalIngredients(
				CUCUMBER,
				ONION,
				CABBAGE,
				TOMATO,
				...
				TURNIP,
				SALT);//Shop
		addMixerRecipe(Meal.LATTE_VEGETABLE, 1.5F, 0.5F, VEGETABLE_JUICE_BASE, MILK).setOptionalIngredients(
				CUCUMBER,
				ONION,
				CABBAGE,
				TOMATO,
		...
		addMixerRecipe(Meal.JUICE_FRUIT, 1.05F, 0.5F, FRUIT_JUICE_BASE).setOptionalIngredients(FRUITS)
				.setMaximumOptionalIngredients(5); //Shops
		addMixerRecipe(Meal.LATTE_FRUIT, 1.04F, 0.6F, FRUIT_JUICE_BASE, MILK).setOptionalIngredients(FRUITS)
				.setMaximumOptionalIngredients(5); //Tomas 20000RP
		addMixerRecipe(Meal.JUICE_MIX, 0.5F, 0.5F, FRUIT_JUICE_BASE, VEGETABLE_JUICE_BASE).setOptionalIngredients(
				FRUITS,
				VEGETABLE_JUICE_BASE,
				TURNIP,
				SALT).setMaximumOptionalIngredients(7); //Shops
		addMixerRecipe(Meal.LATTE_MIX, 1.05F, 0.6F, FRUIT_JUICE_BASE, VEGETABLE_JUICE_BASE, MILK).setOptionalIngredients(
				FRUITS,
				VEGETABLE_JUICE_BASE,
				SALT).setMaximumOptionalIngredients(7);//Candice 20000RP
		//Vanilla style
	...
	private static void addNoUtensilRecipes() {
		...
		addNoUtensilRecipe(Meal.SALAD, 1.1F, 1.2F, SALAD_BASE).setOptionalIngredients(
				MUSHROOM,
				CUCUMBER,
				CABBAGE,
				TOMATO,
				CARROT,
				SALT); //Jenni 5000RP
		addNoUtensilRecipe(Meal.SANDWICH, 1.2F, 1.1F, BREAD, SANDWICH_BASE).setOptionalIngredients(
				BUTTER,
				TOMATO,
				CUCUMBER,
				SALT,
		...
		addNoUtensilRecipe(
				...
				LARGE_EGG,
				OIL);//Danieru 20000RP
		addNoUtensilRecipe(Meal.SANDWICH_FRUIT, 1.05F, 1F, BREAD, FRUITS).setOptionalIngredients(FRUITS)
				.setMaximumOptionalIngredients(5);//Shops
		...
		addNoUtensilRecipe(Meal.ICE_CREAM, 1.8F, 0.8F, MILK, EGG).setOptionalIngredients(
				PINEAPPLE,
				ORANGE,
				STRAWBERRY,
				GRAPE,
				PEACH,
				BANANA); //Candice 10000RP#
	...
	private static void addPotRecipes() {
		...
		addPotRecipe(Meal.STEW, 1.5F, 1.5F, MILK, FLOUR).setOptionalIngredients(
				EGGPLANT,
				ONION,
				POTATO,
				CARROT,
	...
	private static void addOvenRecipes() {
		...
		addOvenRecipe(Meal.CAKE_CHOCOLATE, 1.3F, 1.1F, EGG, FLOUR, BUTTER, CHOCOLATE).setOptionalIngredients(
				SUGAR,
				FRUITS); //Yulif RP 100000
		//Added in 0.6+
		...
		addOvenRecipe(Meal.CAKE, 1.3F, 1.1F, EGG, FLOUR, BUTTER, CAKE_FRUIT).setOptionalIngredients(
				ORANGE,
				PINEAPPLE,
				STRAWBERRY,
				PEACH,
