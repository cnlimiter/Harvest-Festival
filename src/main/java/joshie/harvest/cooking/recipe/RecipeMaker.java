...
public class RecipeMaker implements CookingHandler {
	...
	private NonNullList<ItemStack> findRecipe(
			Utensil utensil,
			Recipe recipe,
			List<IngredientStack> ingredients,
			boolean onlyRequiredAllowed) {
		if (recipe.getUtensil() != utensil) {
			return null;
		}
		if (onlyRequiredAllowed && areOnlyRequiredInRecipe(recipe.getRequired(), ingredients)) {
			return null;
		}
		if (!areIngredientsAllowedInRecipe(recipe, ingredients)) {
			return null;
		}
		if (!areAllRequiredInRecipe(recipe.getRequired(), ingredients)) {
			return null;
		}
	...
	private static boolean areOnlyRequiredInRecipe(Collection<IngredientStack> requiredSet, Collection<IngredientStack> ingredients) {
		for (IngredientStack ingredient : ingredients)
			if (ingredient.isSame(requiredSet)) {
				return false;
			}
	...
	public static boolean areAllRequiredInRecipe(Collection<IngredientStack> requiredSet, Collection<IngredientStack> ingredients) {
		for (IngredientStack required : requiredSet)
			if (!required.isSame(ingredients)) {
				return false;
			}
		return true; //Return true because they were all in the recipe!
	}
	...
	private boolean areIngredientsAllowedInRecipe(Recipe recipe, List<IngredientStack> ingredients) {
		for (IngredientStack ingredient : ingredients)
			if (!isInRecipe(recipe, ingredient)) {
				return false;
			}
