...
public class TileOven extends TileCookingTicking {
		...
		if (giveTimer > 0) {
			...
			if (giveTimer <= 0) {
				for (ItemStack stack : getResult()) {
					if (stack.hasTagCompound() && givePlayer instanceof EntityPlayerMP) {
						EventTrigger.INSTANCE.trigger((EntityPlayerMP) givePlayer, "cooking");
					}
	...
	public void animate() {
		...
		if (getCookTimer() == 1) {
			world.playSound(null, getPos(), HFSounds.OVEN, SoundCategory.BLOCKS, 2F, 1F);
