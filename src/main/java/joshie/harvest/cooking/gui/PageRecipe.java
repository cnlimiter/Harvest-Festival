...
public class PageRecipe extends Page {
	...
	private PageRecipe(Recipe recipe) {
		...
		this.description = recipe.getResource().getResourceDomain() + ".meal." + recipe.getResource().getResourcePath().replace("_", ".") +
				".description";
	}
	...
	public boolean mouseClicked(int mouseX, int mouseY) {
		if (mouseX >= buttonX && mouseX <= buttonX + 66 && mouseY >= buttonY && mouseY <= buttonY + 34) {
			if (RecipeMaker.areAllRequiredInRecipe(recipe.getRequired(), GuiCookbook.ingredients)) {
				String utensil =
						TextFormatting.YELLOW + PageRecipeList.get(recipe.getUtensil()).getItem().getDisplayName() + TextFormatting.RESET;
				...
				if (result == PlaceIngredientResult.SUCCESS) {
					...
					ChatHelper.displayChat(TextFormatting.GREEN + TextHelper.translate("meal.success") + TextFormatting.WHITE + " " +
							TextHelper.format("harvestfestival.meal.success.description", utensil, name));
				} else {
					ChatHelper.displayChat(TextFormatting.RED + TextHelper.translate("meal." + result.name().toLowerCase(Locale.ENGLISH)) +
							TextFormatting.WHITE + "\n " + TextHelper.format(
							"harvestfestival.meal." + result.name().toLowerCase(Locale.ENGLISH) + ".description",
							utensil,
							name));
				}
			} else {
				ChatHelper.displayChat(TextFormatting.RED + TextHelper.translate("meal.missing") + TextFormatting.WHITE + " " +
						TextHelper.translate("meal.missing.description"));
			}
		...
		public void render(GuiCookbook gui, int mouseX, int mouseY) {
			if (stacks.size() > 0) {
				...
				if (isInInventory) {
					...
					gui.drawTexture(x + 9, y + 10, 41, 248, 7, 8);
				}
				...
				stacks = CookingAPI.INSTANCE.getStacksForIngredient(ingredient.getIngredient());
			}
		}
	}
}
