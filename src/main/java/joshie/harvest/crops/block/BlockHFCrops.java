...
public class BlockHFCrops extends BlockHFEnum<BlockHFCrops, CropType> implements IPlantable, IGrowable, IAnimalFeeder {
	...
	public float getBlockHardness(IBlockState state, World world, BlockPos pos) {
		...
		if (type.isWithered()) {
			...
			return isWood(world, pos) ? 2F : 0.1F;
		}
	}
	...
	public boolean isWood(IBlockAccess world, BlockPos pos) {
		...
		if (data != null && data.getCrop() instanceof Tree) {
			if (data.getStage() >= ((Tree) data.getCrop()).getStagesToMaturity()) {
				return true;
			}
		}
	...
	public boolean onBlockActivated(
		...
		if (stage == CropType.WITHERED || stage == CropType.WITHERED_DOUBLE) {
			return false; //If Withered with suck!
		}
	...
	public void breakBlock(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state) {
		...
		if (stage == FRESH || stage == CropType.WITHERED) {
			...
			if (data == null || data.isClearable()) {
				...
				if (world.getBlockState(pos.up()).getBlock() == this && !data.getCrop().isCurrentlyDouble(data.getStage())) {
					data.markSafe(world, pos.up(), PlantSection.TOP);
				}
	...
	public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess world, BlockPos pos) {
		CropType stage = getEnumFromState(state);
		CropData data = CropHelper.getCropDataAt(world, pos);
		return data != null ? data.getCrop().getStateHandler().getBoundingBox(
				world,
				pos,
				stage.getSection(),
				data.getCrop(),
				data.getStage(),
				stage.isWithered()) : CROP_AABB;
	}
	...
	public AxisAlignedBB getCollisionBoundingBox(IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos) {
		CropType stage = getEnumFromState(state);
		CropData data = CropHelper.getCropDataAt(world, pos);
		return data != null ? data.getCrop().getStateHandler().getCollisionBoundingBox(
				world,
				pos,
				stage.getSection(),
				data.getCrop(),
				data.getStage(),
				stage.isWithered()) : NULL_AABB;
	}
	...
	public ItemStack getPickBlock(
			...
			@Nonnull BlockPos pos,
			EntityPlayer player) {
		if (getEnumFromState(state) == CropType.WITHERED) {
			return new ItemStack(Blocks.DEADBUSH); //It's Dead soo???
		}
		CropData data = CropHelper.getCropDataAt(world, pos);
	...
	public IBlockState getActualState(@Nonnull IBlockState state, IBlockAccess world, BlockPos pos) {
		...
		if (crop != null) {
			return crop.getData().getCrop().getStateHandler().getState(
					world,
					pos,
					stage.getSection(),
					crop.getData().getCrop(),
					crop.getData().getStage(),
					stage.isWithered());
		} else {
	...
	public boolean canGrow(@Nonnull World world, @Nonnull BlockPos pos, @Nonnull IBlockState state, boolean isClient) {
		if (getEnumFromState(state) == CropType.WITHERED) {
			return false; //It's dead it can't grow...
		}
