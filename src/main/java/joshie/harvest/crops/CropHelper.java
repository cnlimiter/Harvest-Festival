...
public class CropHelper {
	private static final Cache<BlockPos, IBlockState> RESERVE = CacheBuilder.newBuilder()
	...
	public static TileWithered getTile(IBlockAccess world, BlockPos pos, PlantSection section) {
		if (section == IStateHandler.PlantSection.BOTTOM) {
			...
			TileWithered down = (
					(TileWithered) (
							world instanceof ChunkCache ? ((ChunkCache) world).getTileEntity(
									pos.down(),
									Chunk.EnumCreateEntityType.CHECK) : world.getTileEntity(pos.down())));
			return down == null ? (TileWithered) (
					world instanceof ChunkCache ? ((ChunkCache) world).getTileEntity(
							pos,
							Chunk.EnumCreateEntityType.CHECK) : world.getTileEntity(pos)) : down;
		}
	...
	public static WateringHandler getWateringHandler(World world, BlockPos pos, IBlockState state) {
		for (WateringHandler checker : CropRegistry.INSTANCE.wateringHandlers) {
			if (checker.handlesState(world, pos, state)) {
				return checker;
			}
		}
	...
	public static CropData getCropDataAt(IBlockAccess world, BlockPos pos) {
		...
		if (section == PlantSection.BOTTOM) {
			...
			if (tile instanceof TileWithered) {
	...
	public static boolean isRainingAt(World world, BlockPos pos) {
		...
		return (weather.isRain() || (weather.isSnow() && biome.isHighHumidity())) && world.canBlockSeeSky(pos) &&
				world.getPrecipitationHeight(pos).getY() <= pos.getY() && world.getBiome(pos).canRain();
	}
}