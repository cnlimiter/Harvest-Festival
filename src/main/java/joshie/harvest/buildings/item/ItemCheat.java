		...
		if (damage == Cheat.COORD_SETTER.ordinal()) {
			...
			for (int x = 0; x < 38; x++) {
				for (int z = 0; z < 31; z++) {
					...
					} else if (world.getBlockState(target).getBlock() == Blocks.AIR) {
						states.add(Pair.of(target, Blocks.GRASS_PATH.getDefaultState()));
					}
				}
			}
			...
			for (int x = 0; x < 38; x++) {
				for (int z = 0; z < 31; z++) {
					...
					if (world.getBlockState(target).getBlock() == Blocks.GRASS) {
						...
						states.add(Pair.of(target, Blocks.AIR.getDefaultState()));
					}
				}
			}
				...
				for (int f = 1; f <= 127; f += 42) {
					...
					for (IBlockState state : counter.keySet()) {
						HarvestFestival.LOGGER.log(
								Level.INFO,
								WordUtils.capitalize(state.toString().replace("harvestfestival:ore[ore=", "").replace("]", "")) +
										" with average of " + (counter.get(state) / loop));
					}
				}
			}
		}
		...
		for (Cheat cheat : values) {
			if (cheat != Cheat.BUILDING_PREVIEW) {
				ModelLoader.setCustomModelResourceLocation(
						item,
						cheat.ordinal(),
						new ModelResourceLocation(getRegistryName(), cheat.getName()));
			} else {
				ModelLoader.setCustomModelResourceLocation(
						item,
						cheat.ordinal(),
