...
public class BuildingFestival extends Building {
	...
	public void onFestivalChanged(
			...
			@Nonnull Festival oldFestival,
			@Nonnull Festival newFestival) {
	...
	private void removeOldFestival(Festival oldFestival, World world, BlockPos pos, Rotation rotation, TownDataServer town) {
		if (oldFestival.affectsFestivalGrounds()) {
			getFestivalTemplateFromFestival(oldFestival).removeBlocks(
					world,
					pos,
					rotation,
					Blocks.AIR.getDefaultState(),
					false); //Remove the old festival
		}
		if (oldFestival.getQuest() != null) {
			town.getQuests().removeAsCurrent(world, oldFestival.getQuest());
		}
	}

	private void addNewFestival(Festival newFestival, World world, BlockPos pos, Rotation rotation, TownDataServer town) {
		if (newFestival.affectsFestivalGrounds()) {
			getFestivalTemplateFromFestival(newFestival).placeBlocks(world, pos, rotation, null); //Place the new blocks
		}
		if (newFestival.getQuest() != null) {
			town.getQuests().startQuest(newFestival.getQuest(), true, null);
		}
	}
