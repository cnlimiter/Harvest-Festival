...
public class HFBuildings {
	...
	public static final Building BLACKSMITH = registerBuilding("blacksmith").setRequirements("mining_hill")
			.setInhabitants(HFNPCs.BLACKSMITH)
			.setOffset(3, -2, 6);
	public static final Building CAFE = registerBuilding("cafe").setRequirements("supermarket").setInhabitants(
			HFNPCs.CAFE_OWNER,
			HFNPCs.CAFE_GRANNY).setOffset(7, -1, 10);
	public static final Building CARPENTER = registerBuilding("carpenter").setSpecialRules(NEVER).setInhabitants(
			HFNPCs.CARPENTER,
	...
	public static final Building FISHING_HUT = registerBuilding("fishing_hut").setRequirements("barn", "poultry_farm")
			...
			.setOffset(6, -1, 8);
	public static final Building GODDESS_POND = registerBuilding("goddess_pond").setSpecialRules(new SpecialRuleBuildings(5))
			.setInhabitants(HFNPCs.GODDESS)
			.setOffset(11, -1, 20);
	public static final Building MINING_HILL = registerBuilding("mining_hill").setRequirements("supermarket")
			.setInhabitants(HFNPCs.MINER)
			.setOffset(10, -3, 11);
	public static final Building POULTRY_FARM = registerBuilding("poultry_farm").setInhabitants(HFNPCs.POULTRY).setOffset(4, -1, 12);
	public static final Building SUPERMARKET = registerBuilding("supermarket").setInhabitants(HFNPCs.MILKMAID, HFNPCs.GS_OWNER).setOffset(
			7,
			-1,
			12).setTickTime(5);
	public static final Building TOWNHALL = registerBuilding("townhall").setSpecialRules(new SpecialRuleBuildings(9))
			.setInhabitants(HFNPCs.MAYOR, HFNPCs.PRIEST, HFNPCs.DAUGHTER_ADULT, HFNPCs.DAUGHTER_CHILD)
			.setOffset(10, -1, 17);
	public static final Building FESTIVAL_GROUNDS = registerBuilding(
			"festivals",
			BuildingFestival.class).setSpecialRules(new SpecialRuleFestivals()).setInhabitants(HFNPCs.TRADER).setOffset(14, -1, 32);
		...
		for (Building building : Building.REGISTRY.values()) {
			...
			if (template != null) {
				...
				HarvestFestival.LOGGER.log(
						Level.WARN,
						"Failed to load the template for the building: " + building.getResource().toString());
			}
		}
	}
	...
	private static <B extends Building> B registerBuilding(String name, Class<B>... clazzes) {
		...
		try {
			if (clazz != null) {
				building = clazz.getConstructor(ResourceLocation.class).newInstance(new ResourceLocation("harvestfestival", name));
			}
