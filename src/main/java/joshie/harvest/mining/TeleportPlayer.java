...
public class TeleportPlayer {
	...
	public static boolean isTeleportTargetSetTo(EntityPlayer player, BlockPos target) {
		if (!player.world.isRemote) {
			...
			return CLIENT_TICKER != null && CLIENT_TICKER.location.equals(target);
		}
	}
	...
	private static class TeleportTicker {
		...
		public void onWorldTick(WorldTickEvent event) {
			if (isCollidingWithOrigin()) {
				if (worldTime != 0 && player.world.getTotalWorldTime() == worldTime) {
					return;
				} else {
				...
				if (counter <= 0) {
					...
					if (!player.world.isRemote) {
						if (target != null) {
							...
							float yaw = facing == EnumFacing.WEST ?
									90F :
									facing == EnumFacing.EAST ? -90F : facing == EnumFacing.NORTH ? 180F : 0F;
