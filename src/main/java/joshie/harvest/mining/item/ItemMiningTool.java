...
public class ItemMiningTool extends ItemHFEnum<ItemMiningTool, MiningTool> {
	...
	private NBTTagCompound getLinkData(EntityPlayer player) {
		try {
			return player.world.isRemote ? linkDataClient.get(player, NBTTagCompound::new) : linkDataServer.get(
					player,
					NBTTagCompound::new);
		...
		if (world.provider.getDimension() == HFMining.MINING_ID) {
			...
			if (tool == MiningTool.ELEVATOR_CABLE) {
				if (player.isSneaking()) {
					...
					if (isElevator(world, pos.down())) {
						link(player, world, pos.down(), stack);
					} else if (isElevator(world, pos)) {
						link(player, world, pos, stack);
					}
				}
	...
	public void addInformation(@Nonnull ItemStack stack, EntityPlayer player, List<String> tooltip, boolean advanced) {
		if (getEnumFromStack(stack) == MiningTool.ELEVATOR_CABLE) {
			tooltip.add(TextFormatting.AQUA + TextHelper.translate("elevator.tooltip"));
		}
	}
}
