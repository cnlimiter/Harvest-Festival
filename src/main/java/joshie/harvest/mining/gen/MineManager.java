		...
		if (!generated.contains(mineID)) {
			...
			for (int i = 0; i < 128; i++) {
				...
				if (!foundTorch && EntityHelper.isSpawnable(world, torch)) {
					...
					} else if (!world.isAirBlock(torch.west())) {
						facing = EnumFacing.EAST;
					} else if (!world.isAirBlock(torch.north())) {
						facing = EnumFacing.SOUTH;
					} else if (!world.isAirBlock(torch.south())) {
				...
				if (foundChest && foundTorch && foundSign) {
					break;
				}
			}
		...
		if (ret == null || MiningHelper.getFloor(ret) != floor) {
			...
			for (int x = 0; x < 16 * CHUNK_BOUNDARY; x++) {
				for (int z = 0; z < 16 * CHUNK_BOUNDARY; z++) {
					...
					if (state.getBlock() == HFMining.PORTAL && state.getActualState(world, toCheck)
							.getValue(HFMining.PORTAL.property)
							.isCentre()) {
