...
public class BakedDirt extends BakedHF {
	...
	public List<BakedQuad> getQuads(final @Nullable IBlockState state, final @Nullable EnumFacing side, final long rand) {
		...
		overlay.getQuads(state, side, rand)
				.stream()
				.map(quad -> new BakedQuadRetextured(quad, getRandomTexture(rand)))
	...
	public static class StateMapper extends DefaultStateMapper {
		...
		public Map<IBlockState, ModelResourceLocation> putStateModelLocations(Block block) {
			if (block == HFMining.DIRT) {
				mapStateModelLocations.put(
						new BlockStateContainer(block).getBaseState(),
						new ModelResourceLocation(block.getRegistryName(), "overlay"));
			}
		...
		public void onBaking(ModelBakeEvent event) {
			...
			for (TextureType type : TextureType.values()) {
				TextureAtlasSprite sprite = Minecraft.getMinecraft().getTextureMapBlocks().getAtlasSprite(
						"harvestfestival:blocks/mine/overlays/" + type.name().toLowerCase(Locale.ENGLISH));
				...
				for (BlockDirt.TextureStyle nw : TextureStyle.values()) {
					for (BlockDirt.TextureStyle se : TextureStyle.values()) {
						for (BlockDirt.TextureStyle sw : TextureStyle.values()) {
							...
							registry.putObject(
									new ModelResourceLocation("harvestfestival:dirt", state),
									new BakedDirt(original, overlay, overlays));
						}
					}
				}
			}
