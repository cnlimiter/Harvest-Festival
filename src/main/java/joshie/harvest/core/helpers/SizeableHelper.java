...
public class SizeableHelper {
	public static NonNullList<ItemStack> getSizeablesForDisplay(AnimalStats stats, Sizeable type) {
		...
		for (Size size : Size.values()) {
			...
			int value = RelationshipType.ANIMAL.getMaximumRP() -
					(RelationshipType.ANIMAL.getMaximumRP() - (stats.getHappiness() - size.getRelationshipRequirement()));
			if (value > 0 || (
					size == Size.LARGE && stats.performTest(AnimalTest.WON_CONTEST) &&
							stats.getHappiness() >= (Size.LARGE.getRelationshipRequirement() + 3000))) {
	...
	private static ItemStack getSizeable(AnimalStats stats, Sizeable sizeable, int size) {
		if (stats.performTest(AnimalTest.WON_CONTEST) && stats.getHappiness() >= (Size.LARGE.getRelationshipRequirement() + 3000) &&
				stats.getAnimal().world.rand.nextInt(100) == 0) {
	...
	private static Size getSizeFromAnimal(int happiness, EntityAnimal animal) {
		...
		for (Size size : Size.values()) {
			...
			int value = RelationshipType.ANIMAL.getMaximumRP() -
					(RelationshipType.ANIMAL.getMaximumRP() - (happiness - size.getRelationshipRequirement()));
			if (value > 0) {
