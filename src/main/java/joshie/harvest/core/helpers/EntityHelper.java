	...
	public static <T extends Entity> List<T> getEntities(Class<? extends T> t, World world, BlockPos pos, double size, double ySize) {
		return world.getEntitiesWithinAABB(
				...
				new AxisAlignedBB(
						...
						pos.getX() + 0.5F,
						pos.getY() + 0.5F,
						pos.getZ() + 0.5F).expand(size, ySize, size));
	}
	...
	public static <T extends Entity> List<T> getEntities(Class<? extends T> t, Entity entity, double size) {
		return entity.world.getEntitiesWithinAABB(
				...
				new AxisAlignedBB(
						...
						entity.posX + 0.5F,
						entity.posY + 0.5F,
						entity.posZ + 0.5F).expand(size, 3D, size));
	}
			...
			if (entity instanceof EntityPlayer) {
				...
				if (!player.world.isRemote) {
					ObfuscationReflectionHelper.setPrivateValue(
							...
							"field_184851_cj");
					newWorld.getMinecraftServer().getPlayerList().transferPlayerToDimension(
							player,
							dimension,
							new HFTeleporter(newWorld, spawn));
