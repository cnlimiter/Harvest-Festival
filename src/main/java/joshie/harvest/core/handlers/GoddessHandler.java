...
public class GoddessHandler {
	...
	private static boolean isGoddessFlower(@Nonnull ItemStack stack) {
		if (goddess == null) {
			goddess = Item.getItemFromBlock(HFCore.FLOWERS);
		}
	...
	private static boolean spawnGoddess(World world, double x, double y, double z, boolean flower, boolean move) {
		...
		if (move || (goddess.posX == 0 && goddess.posY == 0 && goddess.posZ == 0)) {
	...
	private void checkFlower(World world, BlockPos pos, EntityPlayer player) {
		...
		if (xMinus == Blocks.WATER) {
			water++;
		}
		if (xPlus == Blocks.WATER) {
			water++;
		}
		if (zMinus == Blocks.WATER) {
			water++;
		}
		if (zPlus == Blocks.WATER) {
			water++;
		}
