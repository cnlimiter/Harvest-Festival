...
public class GuiHandler implements IGuiHandler {
	...
	public Object getServerGuiElement(int ID, EntityPlayer player, World world, int entityID, int nextGui, int hand) {
		...
		switch (ID) {
			case SELECTION:
			case NPC:
				return new ContainerNPCChat(player, (EntityNPC) world.getEntityByID(entityID), nextGui, false);
			case NPC_INFO:
			case SHOP_OPTIONS:
				return new ContainerNPCChat(player, (EntityNPC) world.getEntityByID(entityID), nextGui, true);
			case SHOP_WELCOME:
				return new ContainerNPCChat(player, (EntityNPC) world.getEntityByID(entityID), SHOP_OPTIONS, true);
			case SHOP_MENU:
			case SHOP_MENU_SELL:
			...
			case GIFT:
				return new ContainerNPCGift(player, (EntityNPC) world.getEntityByID(entityID), EnumHand.values()[hand], nextGui);
			case FRIDGE:
				return new ContainerFridge(
						player,
						player.inventory,
						(TileFridge) world.getTileEntity(new BlockPos(entityID, nextGui, hand)));
			case MAILBOX:
			case GIFT_GODDESS:
			case FORCED_NPC:
			case QUEST_BOARD:
			...
			case BASKET:
				return new ContainerBasket(player.inventory, player.getHeldItem(EnumHand.values()[hand]), null);
			case BASKET_ENTITY:
				return new ContainerBasket(player.inventory, ItemStack.EMPTY, BasketHandler.getWearingBasket(player));
			default:
				return null;
		}
	}
	...
	public Object getClientGuiElement(int ID, EntityPlayer player, World world, int entityID, int nextGui, int hand) {
		switch (ID) {
			...
			case NPC: {
				...
				if (hand != -1) {
					return new GuiNPCSelect(player, npc, nextGui, hand);
				}
			...
			case SHOP_WELCOME:
				return new GuiNPCChat(player, (EntityNPC) world.getEntityByID(entityID), SHOP_OPTIONS, false, true);
			case SHOP_MENU_SELL:
			case SHOP_MENU:
				HFApi.player.getTrackingForPlayer(player).learnNote(HFNotes.SHOPPING);
				return new GuiNPCShop(player, (EntityNPC) world.getEntityByID(entityID), NEXT_NONE, ID == SHOP_MENU_SELL);
			case GIFT:
				return new GuiNPCGift(player, (EntityNPC) world.getEntityByID(entityID), EnumHand.values()[hand]);
			case GIFT_GODDESS:
				return new GuiNPCGift(player, (EntityNPC) world.getEntityByID(entityID), GuiNPCGift.GODDESS_GIFT);
			case FRIDGE:
				return new GuiFridge(player, player.inventory, (TileFridge) world.getTileEntity(new BlockPos(entityID, nextGui, hand)));
			case QUEST_BOARD:
				return new GuiQuestBoard(new BlockPos(entityID, nextGui, hand), player);
			case MAILBOX:
			...
			case SHOP_OPTIONS: {
				...
				if (NPCHelper.isShopOpen(npc, world, player)) {
					...
					return new GuiNPCChat(player, npc, nextGui, false, true);
				}
			}

			case BASKET:
				return new GuiBasket(player.inventory, player.getHeldItem(EnumHand.values()[hand]), null);
			case BASKET_ENTITY:
				return new GuiBasket(player.inventory, ItemStack.EMPTY, BasketHandler.getWearingBasket(player));
			default:
				return null;
		}
	}
