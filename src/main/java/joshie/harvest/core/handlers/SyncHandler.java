...
public class SyncHandler {
	...
	public void onPlayerLogin(PlayerLoggedInEvent event) {
		if (event.player instanceof EntityPlayerMP) {
			...
			HFTrackers.<PlayerTrackerServer>getPlayerTrackerFromPlayer(player)
					.getStats()
					.setBirthday(player.world); //Set birthday to overworld date
	...
	public void onChangeDimension(PlayerChangedDimensionEvent event) {
		if (event.player instanceof EntityPlayerMP && event.toDim == 0) {
			HFTrackers.<TownTrackerServer>getTowns(MCServerHelper.getWorld(event.toDim))
					.syncToPlayer((EntityPlayerMP) event.player); //Resync the town data
		}
	}
}