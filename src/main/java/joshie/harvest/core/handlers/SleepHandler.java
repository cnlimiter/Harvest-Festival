...
public class SleepHandler {
	...
	private SleepResult trySleep(EntityPlayer player, BlockPos bedLocation) {
		if (!player.world.isRemote) {
			...
			if (Math.abs(player.posX - bedLocation.getX()) > 3.0D || Math.abs(player.posY - bedLocation.getY()) > 2.0D || Math.abs(
					player.posZ - bedLocation.getZ()) > 3.0D) {
			...
			List<EntityMob> list = player.world.getEntitiesWithinAABB(
					EntityMob.class,
					new AxisAlignedBB(
							bedLocation.getX() - 8.0D,
							bedLocation.getY() - 5.0D,
							bedLocation.getZ() - 8.0D,
							bedLocation.getX() + 8.0D,
							bedLocation.getY() + 5.0D,
							bedLocation.getZ() + 8.0D));
		...
		if (player.world.isBlockLoaded(bedLocation)) {
			state = player.world.getBlockState(bedLocation);
		}
	...
	private void setSize(EntityPlayer player, float width, float height) {
		if (width != player.width || height != player.height) {
			...
			player.setEntityBoundingBox(new AxisAlignedBB(
					...
					axisalignedbb.minY,
					axisalignedbb.minZ,
					axisalignedbb.minX + player.width,
					axisalignedbb.minY + player.height,
