...
public class HolderRegistryMulti<R> {
	...
	public void register(Object object, R r) {
		if (object instanceof Item) {
			registerHolder((Item) object, ItemHolder.of((Item) object), r);
		}
	...
	public NonNullList<ItemStack> getStacksFor(R ingredient) {
		...
		registry.entrySet().stream().filter(entry -> isEqual(ingredient, entry.getValue())).forEach(entry -> result.addAll(entry.getKey()
				.getMatchingStacks()));
		return result;
	}
