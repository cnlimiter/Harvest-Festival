...
public class ContainerExpanded extends ContainerBase {
	...
	public ItemStack slotClick(int slotID, int dragType, ClickType clickTypeIn, EntityPlayer player) {
		...
		return slot instanceof SlotHF ? specialClick(slotID, dragType, clickTypeIn, player) : super.slotClick(
				slotID,
				dragType,
				clickTypeIn,
				player);
	...
	private int getMaxStackSize(@Nonnull ItemStack stack, boolean reverse) {
		if (reverse) {
			return getMaximumStorage(stack.getMaxStackSize());
		}
	...
	private static boolean areItemStacksEqual(@Nonnull ItemStack stackA, @Nonnull ItemStack stackB) {
		return stackB.getItem() == stackA.getItem() && (!stackA.getHasSubtypes() || stackA.getMetadata() == stackB.getMetadata()) &&
	...
	public ItemStack specialClick(int slotId, int dragType, ClickType clickType, EntityPlayer player) {
		...
		if (clickType == ClickType.QUICK_CRAFT) {
			...
			if ((i != 1 || dragEvent != 2) && i != dragEvent) {
				...
				if (slot != null && canAddItemToSlot(slot, itemstack1, true) && slot.isItemValid(itemstack1) &&
						(dragMode == 2 || itemstack1.getCount() > dragSlots.size()) && canDragIntoSlot(slot)) {
					dragSlots.add(slot);
				}
			} else if (dragEvent == 2) {
				if (!dragSlots.isEmpty()) {
					...
					for (Slot slot1 : dragSlots) {
						...
						if (slot1 != null && canAddItemToSlot(slot1, itemstack2, true) && slot1.isItemValid(itemstack2) &&
								(dragMode == 2 || itemstack2.getCount() >= dragSlots.size()) && canDragIntoSlot(slot1)) {
				...
				if (slot7 != null) {
					...
					if (inSlot.isEmpty()) {
						...
						if (held.isEmpty()) {
							...
							if (inSlot.getItem() == held.getItem() && inSlot.getMetadata() == held.getMetadata() &&
						...
						} else if (inSlot.getItem() == held.getItem() && held.getMaxStackSize() > 1 &&
								(!inSlot.getHasSubtypes() || inSlot.getMetadata() == held.getMetadata()) && ItemStack.areItemStackTagsEqual(
								inSlot,
								held) && !inSlot.isEmpty()) {
		...
		} else if (clickType == ClickType.CLONE && player.capabilities.isCreativeMode && inventory.getItemStack().isEmpty() &&
				slotId >= 0) {
			...
			if (!itemstack6.isEmpty() && (slot2 == null || !slot2.getHasStack() || !slot2.canTakeStack(player))) {
				...
				for (int i3 = 0; i3 < 2; ++i3) {
					for (
							int j3 = i1;
							j3 >= 0 && j3 < inventorySlots.size() && itemstack6.getCount() < itemstack6.getMaxStackSize(); j3 += j1) {
						Slot slot8 = inventorySlots.get(j3);

						if (slot8.getHasStack() && canAddItemToSlot(slot8, itemstack6, true) && slot8.canTakeStack(player) && canMergeSlot(
								itemstack6,
								slot8)) {
